      ...
      <div style={{ display:'flex', flexDirection:'column', gap:'var(--space-8)', maxWidth:600 }}>
        {SETTING_SECTIONS.map((section) => (
          <div key={section.title}>
            <h2 style={{ fontSize:'13px', fontWeight:'700', color:'var(--color-text-muted)', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:'var(--space-3)' }}>
              {section.title}
            </h2>
            <div data-card style={{ overflow:'hidden' }}>
              {section.items.map((item, i) => (
                <button key={item.label} type="button"
                  ...
                  onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = 'var(--color-bg-sunk)'; }}
                  onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = 'var(--color-surface)'; }}>
                  <span style={{ width:40, height:40, background:'var(--color-bg-sunk)', borderRadius:'var(--radius-md)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, color:'var(--color-primary)' }}>
                    <item.icon size={20} />
                  </span>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontSize:'13px', fontWeight:'500', marginBottom:2 }}>{item.label}</div>
