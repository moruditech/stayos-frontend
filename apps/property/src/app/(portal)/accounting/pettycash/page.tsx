...
/**
 * Petty cash floats — part of the Accounting hub (previously at
 * /pettycash/floats; moved here as part of combining Expenses + Petty Cash
 * into one Accounting section).
 *
 * Fixed: reconcile now sends `countedAmount` (matching the backend
 * `reconcileSchema` exactly) — the previous version sent `actualBalance`,
 * a field the backend never recognized, so every reconciliation 422'd.
 */
...
const reconcileSchema = z.object({
  countedAmount: z.coerce.number().min(0, 'Counted amount is required'),
...
export default function PettyCashFloatsPage(): React.ReactElement {
  ...
  // Needed for the custodian picker — every float requires a custodianId.
            ...
            return (
              <div key={id} data-float-card>
                ...
                <div data-float-actions>
                  <Link href={`/accounting/pettycash/${id}/ledger`} data-btn-ghost data-btn-sm>View ledger</Link>
      ...
      <Modal open={!!reconcilingId} onClose={() => setReconcilingId(null)} title="Reconcile float">
        ...
        >
          <div data-form-group>
            ...
            <input id="rc-balance" type="number" min={0} step="0.01" {...reconForm.register('countedAmount')} />
            <InlineError message={reconForm.formState.errors.countedAmount?.message} />
          </div>
          <div data-form-group>
