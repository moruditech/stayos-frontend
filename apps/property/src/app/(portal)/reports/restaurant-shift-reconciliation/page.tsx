...
function ShiftReconciliationInner(): React.ReactElement {
  ...
  const { data, isLoading } = useQuery({
    queryKey: reportKeys.restaurantShiftReconciliation(params),
    queryFn: () => api.restaurantReports.getShiftReconciliation(params),
  });

  return (
    <div data-page="report-restaurant-shift-reconciliation">
      ...
      {isLoading || !data ? (
        ...
        <>
          <div data-stat-grid>
            <ReadOnlyField label="Shifts closed" value={String(data.totalShifts)} />
            <ReadOnlyField label="Total variance" value={fmt(data.totalVariance)} />
            <ReadOnlyField label="Shifts over tolerance" value={String(data.shiftsOverTolerance)} />
          </div>
          ...
          <section data-report-section>
            ...
            {data.byStaff.length === 0 ? (
              ...
              <table data-table>
                <thead><tr><th>Staff</th><th>Shifts</th><th>Total variance</th><th>Total sales</th></tr></thead>
                <tbody>
                  {data.byStaff.map((row, i) => (
                    <tr key={i}>
                      <td>{row.staffName}</td>
                      <td>{row.shiftCount}</td>
                      <td>{fmt(row.totalVariance)}</td>
...
export default function ShiftReconciliationReportPage(): React.ReactElement {
  return (
    <RoleGate
