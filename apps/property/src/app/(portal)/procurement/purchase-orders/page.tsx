              ...
              return (
                <tr key={id}>
                  <td><Link href={`/procurement/purchase-orders/${id}`} data-table-link>
                    {String(o['poNumber'] ?? `PO-${id.slice(-6).toUpperCase()}`)}
                  </Link></td>
                  <td>{supplierNameOf(o['supplierId'])}</td>
                  <td>{o['createdAt'] ? fmtDate(String(o['createdAt'])) : '—'}</td>
                  <td>{total}</td>
                  <td>
                    <StatusBadge status={status} />
                    {Boolean(o['autoGenerated']) && <span data-auto-badge title="Automatically generated by auto-restock"> Auto</span>}
                  </td>
                  <td>
